    ...
    axis2_conf_t * conf)
{
	...
	if (!NIL_P(policy_string))
   	{
        int length = -1;
		policy_str = RSTRING_PTR(policy_string);
		length = RSTRING_LEN(policy_string);
                                                ...
                                                VALUE security_token)
{
   ...
   {
        prv_key = RSTRING_PTR(private_key);
   ...
   {
        receiver_certificate = RSTRING_PTR(rec_cert);
   ...
   {
        password_type = RSTRING_PTR(pwd_type);
   ...
   {
 		callback_function_name = RSTRING_PTR(callback_fn);
						...
						VALUE policy)
{
    ...
    if (neethi_options) 
	{
        ...
        { 
            encrypt = RSTRING_PTR(encryption);
        ...
        {
            algo_suite = RSTRING_PTR(algorithm);
        ...
        {
            tkn = RSTRING_PTR(token_reference);
            if (tkn != NULL)
			{
		...
		{
			encrypt_sign = RSTRING_PTR(encrypt_signature);
        ...
        {
            order = RSTRING_PTR(protection_order);
