    ...
    if(TYPE(options) == T_HASH)
    {
        ...
        if(wsdl != Qnil)
        {
            ...
            wsf_ruby_home_string = 
                (axis2_char_t*)axutil_strdup(ws_env_svr, RSTRING_PTR(wsf_ruby_home));

			wsdl_location_string = (axis2_char_t*)axutil_strdup(ws_env_svr, RSTRING_PTR(wsdl));

            if (service_name_value != Qnil)
            {
                service_name = (axis2_char_t*)axutil_strdup(ws_env_svr, 
                                                            RSTRING_PTR(service_name_value));
            }
            
            if (port_name_value != Qnil)
            {
                port_name = (axis2_char_t*)axutil_strdup(ws_env_svr, RSTRING_PTR(port_name_value));
            }
...
wsf_ruby_req_info_fill(wsf_req_info_t *req_info, VALUE request)
{
    ...
    if (request && strcmp(rails_class, rb_class2name(CLASS_OF(request))) == 0) 
    {
        ...
        if(v_rawpost != Qnil)
        {
			...
			request_envelope_len = RSTRING_LEN(v_rawpost);
        }
        else
        {
    ...
    else 
    {
        ...
        req_info->req_data = axutil_strdup(ws_env_svr, RSTRING_PTR(request));
        req_info->req_data_length = RSTRING_LEN(request);
