...
wsdl_data_to_axiom_node(const axutil_env_t* env, axis2_char_t* local_name,
        wsf_wsdl_data_t* data, axiom_node_t* node, axiom_node_t *root_node,
        int *plevel, axutil_hash_t *namespace_map);
...
wsdl_data_util_create_payload(const axutil_env_t* env, wsf_wsdl_data_t* data, int binding_style)
{
    ...
    for (hi = axutil_hash_first(hash, env); hi; hi = axutil_hash_next(env, hi)) 
    {
        ...
        axutil_hash_this(hi, (const void**)&key, &size, &val);
        ...
        if (sub_data)
        {
            ...
            /* create the namespace map */
            namespace_map = axutil_hash_make(env);
            wsdl_data_to_axiom_node(env, key, sub_data, root_node, root_node, &level, namespace_map);

            /* free the namespace map */
            for (hi = axutil_hash_first(namespace_map, env); hi; hi = axutil_hash_next(env, hi)) {
                axutil_hash_this(hi, (const void**)&prefix, &size, &ns_uri);
                AXIS2_FREE(env->allocator, prefix);
                AXIS2_FREE(env->allocator, ns_uri);
            }
            axutil_hash_free(namespace_map, env);
                  ...
                  wsf_wsdl_data_template_t* templ)
{
    ...
    if (element)
    {
        ...
        while (itr && axiom_children_qname_iterator_has_next(itr, env) == AXIS2_TRUE)
        {
                                 ...
                                 wsf_wsdl_data_template_t** templ)
{
    ...
    wsdl_data_template_add_complex_element(env, template_to_return, wrapper_element, NULL, wrapper_element_ns, AXIS2_FALSE, 1, 1, sub_template_to_return, 0);
    ...
    if (first_parse_iter)
    {
        while (axiom_children_iterator_has_next(first_parse_iter, env))
        {
            ...
            if (om_node)
            {
                ...
                qname = axiom_element_get_qname(om_element, env, om_node);
            ...
            int *plevel, axutil_hash_t *namespace_map)
{
    ...
    { 
        ...
        if ((data->children_type == CHILDREN_TYPE_NONE) || (data->children_type == CHILDREN_TYPE_ATTRIBUTES))
        {
            if (data->target_ns)
            {
                prefix = axutil_hash_get(namespace_map, data->target_ns, AXIS2_HASH_KEY_STRING);
                if(!prefix)
                {
                    axiom_namespace_t *new_ns = NULL;
                    prefix = AXIS2_MALLOC(env->allocator, sizeof(axis2_char_t) * 16);
                    sprintf(prefix, "ns%d", (*plevel)++);
                    axutil_hash_set(namespace_map, data->target_ns, AXIS2_HASH_KEY_STRING, prefix);

                    new_ns = axiom_namespace_create(env, data->target_ns, prefix);
                    
                    axiom_element_declare_namespace(
                          (axiom_element_t*)axiom_node_get_data_element(root_node, env),
                          env, root_node, new_ns);

                }
                target_namespace = axiom_namespace_create(env, (const axis2_char_t*)data->target_ns, prefix);
            }
            else
            {
            ...
            if(target_namespace) {
                axiom_element_set_namespace(current_element, env, target_namespace, current_node);
            }
        ...
        while (axutil_linked_list_size(link_list, env) > 0)
        {
            ...
            if (pair)
            {
                ...
                {
                    wsdl_data_to_axiom_node(env, pair->name, pair->data, current_node, root_node, plevel, namespace_map);
                }
            }
...
validate_complex(const axutil_env_t* env, axiom_node_t* type_map, wsf_wsdl_data_template_t* templ, wsf_wsdl_data_t* data, int validation_criteria)
{
    ...
    if (templ->is_simple == AXIS2_FALSE)
    {
        ...
        if (validation_criteria == VALIDATION_CRITERIA_REQUEST_MODE_TYPE_WITH_NS)
        {
            ...
            {
				AXIS2_LOG_ERROR (env->log, AXIS2_LOG_SI, "%s namespace expected %s, given %s",
					AXIS2_LOG_PROJECT_PREFIX, templ->type_ns, data->type_ns);
                return AXIS2_FALSE;
            }
        }
        else {
            data->type_ns = axutil_strdup(env, templ->type_ns);
            data->target_ns = axutil_strdup(env, templ->target_ns);
        }
    
        if (templ->data == NULL)
        {
			AXIS2_LOG_ERROR (env->log, AXIS2_LOG_SI, "%s Data is NULL",
					AXIS2_LOG_PROJECT_PREFIX);
            return AXIS2_FALSE;
        }
        ...
        for (hi = axutil_hash_first(hash, env); hi; hi = axutil_hash_next(env, hi)) 
        {
            ...
            if (templ_sub)
            {
                if (data->children_type == CHILDREN_TYPE_ATTRIBUTES)
                {
                    ...
                    else /* child not NULL */
                    {
                        if (data_sub->children_type == CHILDREN_TYPE_ARRAY_ELEMENTS)
                        {
                            ...
                            {
								AXIS2_LOG_ERROR (env->log, AXIS2_LOG_SI, "%s Violates maxOccurs",
										AXIS2_LOG_PROJECT_PREFIX);
                                return AXIS2_FALSE;
                            }
                ...
                {
					AXIS2_LOG_ERROR (env->log, AXIS2_LOG_SI, "%s Unknown children type",
							AXIS2_LOG_PROJECT_PREFIX);
                    return AXIS2_FALSE;
                }
            }
            ...
            {
				AXIS2_LOG_ERROR (env->log, AXIS2_LOG_SI, "%s template sub is null",
						AXIS2_LOG_PROJECT_PREFIX);
                return AXIS2_FALSE;
            }
            ...
            if (success == AXIS2_FALSE)
            {
				AXIS2_LOG_ERROR (env->log, AXIS2_LOG_SI, "%s failed calling the function recursively",
						AXIS2_LOG_PROJECT_PREFIX);
                return AXIS2_FALSE;
            }
        } /* for */
