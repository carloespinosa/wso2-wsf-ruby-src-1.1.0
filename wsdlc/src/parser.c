...
#include "../include/wsf_wsdl_info.h"
...
{
    return wsf_wsdl_util_get_all_policies_from_wsdl_unified(env,
                                                            wsdl_axiom,
                                                            binding_node,
                                                            operation_name,
                                                            is_version1_wsdl,
...
/* caller should free elements in the hash.
* caller should free keys in the hash too. */
axis2_status_t
wsf_wsdl_util_get_all_policies_from_wsdl_unified(const axutil_env_t* env, 
												 axiom_node_t* wsdl_axiom,
												 axiom_node_t* binding_node,
												 const axis2_char_t* operation_name,
												 axis2_bool_t is_version1_wsdl,
...
wsf_wsdl_util_get_binding_node(
    ...
    axiom_node_t** binding_node)
{   
	return wsf_wsdl_util_get_binding_node_unified(env,
                                                  wsdl_axiom,
                                                  service_name,
                                                  port_name,
                                                  is_version1_wsdl,
...
axis2_status_t
wsf_wsdl_util_get_binding_node_unified(
    const axutil_env_t* env,
    axiom_node_t* wsdl_axiom,
    axis2_char_t* service_name,
    axis2_char_t* endpoint_or_port_name, 
    axis2_bool_t  is_version1_wsdl,
    axiom_node_t** binding_node)
{
	axutil_qname_t *qname = NULL;				
	...
	if (root_element)
	{
		...
		if (service_found)
		{
			...
			while (axiom_children_qname_iterator_has_next(port_itr, env))
			{
				...
				if (!endpoint_or_port_name)
				{
					port_found = AXIS2_TRUE;
					break;
				}

				if (strcmp(endpoint_or_port_name,  
    ...
    axis2_bool_t is_service_call)
{
    return wsf_wsdl_util_apply_xslt(env, 
                                    ...
                                    wsdl_axiom, 
                                    sig_axiom);
    /*axis2_char_t* sig_stream = NULL;						
    int sig_stream_len = -1;
    axiom_xml_reader_t* wsdl_xml_reader = NULL;		
    axiom_xml_reader_t* sig_xml_reader = NULL;		
    axiom_document_t* wsdl_document = NULL;			
    axiom_document_t* sig_document = NULL;			
    axiom_stax_builder_t* wsdl_om_builder = NULL;	
    axiom_stax_builder_t* sig_om_builder = NULL;	
    
    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint A1");

    wsdl_xml_reader = axiom_xml_reader_create_for_file(env, wsdl_file_name, NULL); 

    if (!wsdl_xml_reader)
    {
        AXIS2_LOG_ERROR_MSG(env->log, "Failed to create wsdl_xml_reader");
        return AXIS2_FAILURE;
    }

    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint A2");

    wsdl_om_builder = axiom_stax_builder_create(env, wsdl_xml_reader);

    if (!wsdl_om_builder)
    {
        AXIS2_LOG_ERROR_MSG(env->log, "Failed to create wsdl stax builder");
        axiom_xml_reader_free(wsdl_xml_reader, env);
        return AXIS2_FAILURE;
    }

    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint A3");

    wsdl_document = axiom_stax_builder_get_document(wsdl_om_builder, env);

    if (!wsdl_document)
    {
        AXIS2_LOG_ERROR_MSG(env->log, "Failed to get retrieve wsdl document");
        axiom_stax_builder_free(wsdl_om_builder, env);
        return AXIS2_FAILURE;
    }        

    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint A4");

    *wsdl_axiom = axiom_document_build_all(wsdl_document, env);

    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint A5");

    wsdl_util_apply_xslt(env, 
						 wsdl_file_name, 
						 &sig_stream, 
						 &sig_stream_len, 
						 xslt_location, 
    ...
    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint A6");

    AXIS2_LOG_DEBUG_MSG(env->log, sig_stream);

    sig_xml_reader = axiom_xml_reader_create_for_memory(env, 
                                                        sig_stream, sig_stream_len, 
                                                        NULL, AXIS2_XML_PARSER_TYPE_BUFFER);

    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint 1");
    
    if (!sig_xml_reader)
    {
        AXIS2_LOG_ERROR_MSG(env->log, "Failed to create sig_xml_reader\n");
        return AXIS2_FAILURE; 
    }
    
    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint 2");

    sig_om_builder = axiom_stax_builder_create(env, sig_xml_reader);
    
    if (!sig_om_builder)
    {
        AXIS2_LOG_ERROR_MSG(env->log, "Failed to create sig stax builder\n");
        axiom_xml_reader_free(sig_xml_reader, env);
        return AXIS2_FAILURE;
    }

    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint 3");
    
    sig_document = axiom_stax_builder_get_document(sig_om_builder, env);
    
    if (!sig_document)
    {
        AXIS2_LOG_ERROR_MSG(env->log, "Failed to retrieve sig document\n");
        axiom_stax_builder_free(sig_om_builder, env);
        return AXIS2_FAILURE;
    }

    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint 4");

    *sig_axiom = axiom_document_build_all(sig_document, env);

    AXIS2_LOG_DEBUG_MSG(env->log, "Checkpoint 5");

    AXIS2_LOG_DEBUG (env->log, AXIS2_LOG_SI, "sig_axiom is : \n%s\n", axiom_node_to_string(*sig_axiom, env));
    
	axiom_stax_builder_free_self(wsdl_om_builder, env); 
    axiom_stax_builder_free_self(sig_om_builder, env);
							 ...
							 int* soap_version)
{
    if (is_multiple == AXIS2_TRUE)
    {
        ...
        if (services_element)
        {
            ...
            while (AXIS2_TRUE == axiom_children_qname_iterator_has_next(itr, env))
            {
                axiom_node_t *current_service_node = NULL;
                axiom_element_t *current_service_element = NULL;
                axis2_char_t* address = NULL;

                current_service_node = (axiom_node_t*)axiom_children_qname_iterator_next(itr, env);
                current_service_element = (axiom_element_t*)axiom_node_get_data_element(current_service_node, env);

                /* by default it is the first node */
                if(service_node == NULL) {
                    service_node = current_service_node;
                }
                if(service_element == NULL) {
                    service_element = current_service_element;
                }
        ...
        if(service_node && service_element) 
        {
            axiom_node_t* operations_node = NULL;			
            axiom_element_t* operations_element = NULL;		
            axiom_node_t* operation_node = NULL;
            ...
            while (operation_itr && AXIS2_TRUE == axiom_children_qname_iterator_has_next(operation_itr, env))
            {
                axis2_char_t* operation = NULL;
                operation_node = 
